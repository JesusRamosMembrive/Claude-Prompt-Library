...
def build_request(method: str, url: str, data: dict | None = None) -> urlrequest.Request:
    payload = json.dumps(data).encode("utf-8") if data is not None else None
    headers = {"Content-Type": "application/json"}
    return urlrequest.Request(url=url, data=payload, headers=headers, method=method.upper())
...
def perform_request(req: urlrequest.Request, timeout: float) -> HttpResponse:
    ...
    try:
        with urlrequest.urlopen(req, timeout=timeout) as response:
            ...
            return HttpResponse(status=str(response.status), body=body, latency_ms=(time.perf_counter() - start) * 1000)
    except urlerror.HTTPError as exc:
        try:
            detail = exc.read().decode("utf-8", errors="replace")
        except Exception:
            detail = exc.reason if isinstance(exc.reason, str) else repr(exc.reason)
        return HttpResponse(status=f"HTTP {exc.code}", body=detail, latency_ms=(time.perf_counter() - start) * 1000)
    except socket.timeout:
        return HttpResponse(status="timeout", body="Tiempo de espera agotado.", latency_ms=(time.perf_counter() - start) * 1000)
    except Exception as exc:
        return HttpResponse(status="error", body=str(exc), latency_ms=(time.perf_counter() - start) * 1000)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Depuración de conectividad contra Ollama.")
    parser.add_argument("--endpoint", default=DEFAULT_ENDPOINT, help="Endpoint base de Ollama (por defecto %(default)s)")
    parser.add_argument("--model", default="gpt-oss:latest", help="Modelo a utilizar para la llamada de prueba.")
    parser.add_argument("--prompt", default="Di 'pong' si recibiste este mensaje.", help="Prompt que se enviará al modelo.")
    parser.add_argument("--timeout", type=float, default=10.0, help="Tiempo máximo de espera en segundos.")
...
def main() -> int:
    ...
    parsed = urlrequest.urlparse(endpoint)
    ...
    return 0 if chat_response.status.isdigit() and chat_response.status.startswith("2") else 2
