...
from ..git_history import GitHistory, GitHistoryError, CommitInfo
...
class FileHistoryResponse(BaseModel):
    ...
    commits: List[CommitInfoSchema]
    total_changes: int = Field(..., description="Total number of commits affecting this file")
...
class TimelineMatrixResponse(BaseModel):
    ...
    commits: List[CommitInfoSchema] = Field(..., description="List of commits (columns)")
    files: List[str] = Field(..., description="List of files (rows)")
    matrix: List[List[bool]] = Field(..., description="2D matrix: matrix[file_idx][commit_idx] = file changed")
    total_files: int
    total_commits: int
...
async def get_commits(
    ...
    since: Optional[str] = Query(None, description="ISO date string (e.g., '2025-01-01')"),
    until: Optional[str] = Query(None, description="ISO date string (e.g., '2025-12-31')"),
    ...
    if since:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid 'since' date format: {since}")

    if until:
        try:
            until_dt = datetime.fromisoformat(until)
        except ValueError:
            raise HTTPException(status_code=400, detail=f"Invalid 'until' date format: {until}")
...
async def get_file_history(
    ...
    try:
        ...
        commit_schemas = [
            CommitInfoSchema(**commit_dict)
            for commit_dict in result["commits"]
        ]
...
async def get_timeline_matrix(
    ...
    file_pattern: Optional[str] = Query(None, description="Regex pattern to filter files"),
